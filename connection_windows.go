...
func (ao *AutomationObject) CreateBrowser(rootNode string) (*Tree, error) {
	...
	if rootNode != "" {
		...
		oleutil.MustCallMethod(browser.ToIDispatch(), "MoveTo", rootPath)
	}
...
func NewAutomationObject() *AutomationObject {
	wrappers := []string{"OPC.Automation.1", "Graybox.OPC.DAWrapper.1", "Matrikon.OPC.Automation", "HSCOPC.Automation", "RSI.OPCAutomation"}
	var err error
